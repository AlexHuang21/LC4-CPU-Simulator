...
int main(int argc, char** argv) {
    ...
    //run simulator by running update machine state until PC hits 0x80FF
    while (CPU->PC != 0x80FF) {
        //keep writing out the state until all signals stop
        if (UpdateMachineState(CPU, outputFile) == 1) {
